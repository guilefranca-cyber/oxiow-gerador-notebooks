...
LINHAS = [
    ...
    'import sys, runpy, os',
    '# O proprio erro de CUDA sugere isto: combate a fragmentacao da VRAM.',
    '# Precisa estar definido ANTES de qualquer alocacao na placa.',
    ...
    '',
    '# PATCH 5 (O QUE FALTAVA): o caminho PADRAO do PyTorch tambem faz copy_() em',
    '# tensor meta -> no-op silencioso (o aviso "which is a no-op" era isso).',
    '# O patch 1 cobre o caminho do diffusers; este cobre nn.Module.load_state_dict.',
    '# A prova: "Cannot copy out of meta tensor" em pipeline.to() DEPOIS de',
    '# "atribuidos 242 pesos" — ou seja, sobrou module em meta.',
    '_lfsd_orig = _t.nn.Module._load_from_state_dict',
    'def _lfsd(self, state_dict, prefix, local_metadata, strict,',
    '          missing_keys, unexpected_keys, error_msgs):',
    '    for _n, _p in list(getattr(self, "_parameters", {}).items()):',
    '        if _p is None or str(_p.device.type) != "meta":',
    '            continue',
    '        _novo = state_dict.pop(prefix + _n, None)',
    '        if _novo is not None:',
    '            self._parameters[_n] = _t.nn.Parameter(_novo, requires_grad=_p.requires_grad)',
    '    for _n, _b in list(getattr(self, "_buffers", {}).items()):',
    '        if _b is None or str(getattr(_b, "device", "")) != "meta":',
    '            continue',
    '        _novo = state_dict.pop(prefix + _n, None)',
    '        if _novo is not None:',
    '            self._buffers[_n] = _novo',
    '    return _lfsd_orig(self, state_dict, prefix, local_metadata, strict,',
    '                      missing_keys, unexpected_keys, error_msgs)',
    '_t.nn.Module._load_from_state_dict = _lfsd',
    'print("[patch5] caminho padrao do PyTorch tambem ATRIBUI (meta resolvido)", flush=True)',
    '',
    '# AUTOTESTE (custa 1 segundo e roda ANTES da carga pesada):',
    '# cria um modulo em meta, carrega pesos por load_state_dict e confere que ele',
    '# deixou de ser meta. Se isto passar, o pipeline.to() nao vai mais explodir.',
    'try:',
    '    import torch.nn as _nn',
    '    class _TesteMeta(_nn.Module):',
    '        def __init__(self):',
    '            super().__init__()',
    '            self.a = _nn.Linear(4, 4)',
    '            self.register_buffer("buf", _t.zeros(3))',
    '    _alvo = _TesteMeta().state_dict()',
    '    _mod = _TesteMeta().to("meta")',
    '    _antes = (_mod.a.weight.device.type, _mod.buf.device.type)',
    '    _mod.load_state_dict(_alvo, strict=False)',
    '    _depois = (_mod.a.weight.device.type, _mod.buf.device.type)',
    '    print(f"[autoteste] antes={_antes}  depois={_depois}", flush=True)',
    '    _mod.to("cpu")',
    '    if "meta" in _depois:',
    '        print("[autoteste] FALHOU — ainda em meta; o patch 5 nao pegou", flush=True)',
    '    else:',
    '        print("[autoteste] OK — PATCH 5 FUNCIONA (to(cpu) passou)", flush=True)',
    'except Exception as _e:',
    '    print("[autoteste] erro:", repr(_e)[:150], flush=True)',
    '',
    ...
    '',
    '# PATCH 6 (REESCRITO): a T4 tem 14,56 GB e "pipeline.to(device)" tenta empurrar',
    '# TUDO de uma vez (T5 11,36 GB + CLIP 4,77 GB + 2 transformers + VAE) -> CUDA OOM.',
    '# 1a tentativa usou enable_model_cpu_offload() -> RecursionError, porque o proprio',
    '# offload chama .to() por dentro, que e o metodo interceptado aqui (loop).',
    '# Agora e deterministico: mede cada modulo e sobe para a GPU apenas os leves.',
    '# O T5 (11,36 GB) fica na CPU — ele roda UMA vez, para ler o prompt.',
    'try:',
    '    from diffusers.pipelines.pipeline_utils import DiffusionPipeline as _DP',
    '    _to_orig = _DP.to',
    '    def _to_inteligente(self, *a, **kw):',
    '        movidos, ficaram = [], []',
    '        try:',
    '            itens = list(self.components.items())',
    '        except Exception:',
    '            itens = []',
    '        if not itens:',
    '            return _to_orig(self, *a, **kw)',
    '        for _nome, _mod in itens:',
    '            if _mod is None:',
    '                continue',
    '            try:',
    ...
    '                try:',
    '                    _mod.to("cuda")',
    '                    movidos.append(_nome + "(" + str(round(_gb, 2)) + "GB)")',
    '                except Exception as _em:',
    '                    ficaram.append(_nome + "? " + repr(_em)[:40])',
    '            else:',
    '                ficaram.append(_nome + "(" + str(round(_gb, 2)) + "GB)")',
    '        print("[patch6] NA GPU:", movidos, flush=True)',
    '        print("[patch6] NA CPU:", ficaram, flush=True)',
    '        import torch as _tc',
    '        _tc.cuda.empty_cache()',
    '        return self',
    '    _DP.to = _to_inteligente',
    '    print("[patch6] DiffusionPipeline.to interceptado (movimentacao seletiva)", flush=True)',
    'except Exception as _e:',
    '    print("[patch6] FALHOU:", repr(_e)[:150], flush=True)',
    '',
    '# PATCH 7: o patch 6 deixou o T5 (11,36 GB) na CPU, mas o pipeline faz',
    '# text_input_ids.to(device) com device=CUDA -> "Expected all tensors to be on',
    '# the same device, but got index is on cuda:0, different from other tensors on cpu".',
    '# Aqui forcamos o encode do T5 no device REAL do encoder (CPU) e devolvemos os',
    '# embeds no device que o pipeline pediu (a GPU).',
    'try:',
    '    import src.pipeline_wan_fun_inpaint_audio_2512 as _pm7',
    '    _cls7 = _pm7.WanFunInpaintAudioPipeline',
    '    _gt5 = _cls7._get_t5_prompt_embeds',
    '    def _gt5_novo(self, *a, **kw):',
    '        try:',
    '            _dev_enc = next(self.text_encoder.parameters()).device',
    '        except Exception:',
    '            return _gt5(self, *a, **kw)',
    '        _dev_ped = kw.get("device")',
    '        if _dev_ped is None and len(a) >= 4:',
    '            _dev_ped = a[3]',
    '        if len(a) >= 4:',
    '            _l = list(a)',
    '            _l[3] = _dev_enc',
    '            kw.pop("device", None)',
    '            _r = _gt5(self, *tuple(_l), **kw)',
    '        else:',
    '            kw["device"] = _dev_enc',
    '            _r = _gt5(self, *a, **kw)',
    '        print("[patch7] T5 rodou em", _dev_enc, "| embeds devolvidos em", _dev_ped, flush=True)',
    '        if _dev_ped is not None and str(_dev_ped) != str(_dev_enc):',
    '            if isinstance(_r, tuple):',
    '                return tuple(x.to(_dev_ped) if hasattr(x, "to") else x for x in _r)',
    '            if hasattr(_r, "to"):',
    '                return _r.to(_dev_ped)',
    '        return _r',
    '    _cls7._get_t5_prompt_embeds = _gt5_novo',
    '    print("[patch7] encode do T5 fixado no device real do encoder", flush=True)',
    'except Exception as _e7:',
    '    print("[patch7] FALHOU:", repr(_e7)[:150], flush=True)',
    '',
